...
app.get('/calorie-calculator', (req, res) => {
  res.render('pages/calorie-calculator', { result: null, user: req.user });
});


app.post('/calorie', async (req, res) => {
  const idsRaw = req.body.recipeIds;
  if (!idsRaw) {
    return res.render('pages/calorie-calculator', { result: null, user: req.user });
  }

  const recipeIds = idsRaw.split(',').map(id => id.trim());
  ...
  try {
    ...
    res.render('pages/calorie-calculator', { result, user: req.user });
